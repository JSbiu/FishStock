...
股票与期货自选分别保存在当前 VS Code Profile 的本机 `globalState` 中，卸载或升级扩展不会自动删除。北交所公开证券目录也会保存一份本地缓存，它不包含用户数据。Stock 视图可分别清空或恢复首次运行的“默认”“指数”“银行”三组数据；Futures 视图可独立清空，所有清空或替换操作均需要确认。
