...
0.5.3 是一次首次打开性能优化：本地分组会立即显示，股票、ETF 和期货行情随后在后台补齐；状态栏仍可按用户分组选择参与轮播的范围。FishStock 不要求账户，不上传自选数据，不提供投资建议；行情来自第三方公开网页端点，可能受到地区、网络和访问策略影响。
...
`Stock`、`Fund` 与 `Futures` 是可独立显示或隐藏的 VS Code 原生 View，首次布局提供 `2:1:1` 高度权重。之后 VS Code 会优先恢复每个工作区保存的展开、折叠和高度状态，扩展公开 API 无法强制重设原生分隔位置。
