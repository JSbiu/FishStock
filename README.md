...
0.5.5 是一次原生视图首开性能优化：FishStock 在启动后预初始化 Stock、Fund 与 Futures，重载后即使上次折叠也会提前准备本地条目，避免首次展开时出现数秒空白。FishStock 不要求账户，不上传自选数据，不提供投资建议；行情来自第三方公开网页端点，可能受到地区、网络和访问策略影响。
...
`Stock`、`Fund` 与 `Futures` 是可独立显示或隐藏的 VS Code 原生 View，首次布局提供 `2:1:1` 高度权重。为避免折叠 View 首次展开时出现空白等待，0.5.5 会在窗口启动后预初始化并保持三个 View 标题展开；分组展开状态和 View 高度仍由 VS Code 保存，扩展不强制重设原生分隔位置。
