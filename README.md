...
0.6.1 是行情新鲜度可靠性修订版：自动刷新只请求当前实际交易中的 A 股、港股和国内商品期货条目；每次成功取得有效条目即更新获取时间，不要求价格或行情时间必须变化，短暂失败也不会立即把有效缓存标记为过期。FishStock 不要求账户，不上传自选数据，不提供投资建议；行情来自第三方公开网页端点，可能受到地区、网络和访问策略影响。
