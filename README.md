...
- 自选列表与状态栏共享原生行情浮窗，展示日内价格、成交、PE(TTM) 和总市值
- 腾讯 A 股、港股及市场指数真实行情
...
首次运行会创建三组默认数据：“默认”包含贵州茅台和腾讯控股；“指数”包含上证指数、沪深300和创业板指；“银行”包含工商银行、农业银行、中国银行、建设银行、交通银行和邮储银行。所有条目都可随时删除。
...
自选列表保存在当前 VS Code Profile 的本机 `globalState` 中，卸载或升级扩展不会自动删除。北交所公开证券目录也会保存一份本地缓存，它不包含用户数据。命令面板和 Stock 视图的 `…` 菜单提供两个独立操作：“清空全部自选数据”会保留一个空的“默认”分组；“恢复默认自选数据”会恢复首次运行的“默认”“指数”“银行”三组数据。两者均需要确认。导入、导出只会在用户主动选择文件后进行。
