...
- 按中文名称、拼音简称或代码搜索 A 股与港股，支持科创板和北交所新旧代码，预览结果后添加
- 从视图标题栏或目标分组直接添加股票
- 删除、跨组移动以及上移/下移股票
- 添加、重命名、删除和折叠分组
- 手动刷新与自动刷新
...
自选列表保存在当前 VS Code Profile 的本机 `globalState` 中，卸载或升级扩展不会自动删除。北交所公开证券目录也会保存一份本地缓存，它不包含用户数据。命令面板和 Stock 视图的 `…` 菜单提供两个独立操作：“清空全部自选数据”会保留一个空的“默认”分组；“恢复默认自选数据”会恢复首次运行的默认分组、贵州茅台和腾讯控股。两者均需要确认。导入、导出只会在用户主动选择文件后进行。
