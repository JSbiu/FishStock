...
`Stock`、`Fund` 与 `Futures` 保持为三个可在 VS Code Views 菜单中独立显示或隐藏的原生 View。扩展清单以 `initialSize` 权重 `2:1:1` 提供初始高度。由于 VS Code 会把折叠 View 的内部 Tree 延迟到首次展开时初始化，0.5.5 在启动后以不聚焦、不选中条目的方式预初始化三个 View；它们在窗口重载后保持展开，若启动时正在使用其他侧边栏则在初始化触发后切回。View 隐藏期间的行情树更新会被合并，首次显示直接读取当前内存缓存。高度仍由 VS Code 原生布局维护，用户可拖动或双击分隔线重新分配。
