...
行情模型除现价和昨收外，还可携带今开、最高、最低、成交量、成交额、换手率、PE(TTM) 和总市值。提供器负责把供应商单位转换为基础货币金额，并明确成交量单位；解析器把缺失或不可用的扩展字段转换为 `null`，不因单个估值字段异常丢弃仍然有效的价格行情。Tree View 使用原生 `MarkdownString` 生成紧凑浮窗，不引入 Webview。
