...
- `live`：当前处于交易时段，行情时间在过期阈值内，且最近刷新成功。
- `closed`：午休、盘间休息、收盘、周末或节假日，最近有效行情仍可展示。
- `stale`：交易中行情超时或早于当前时段、最近刷新失败但仍有缓存，或期货夜盘规则尚未收录。
- `error`：没有有效行情可展示，标记“暂不可用”。

`marketSessions` 领域模块根据规范化市场、期货品种和上海时区时间生成阶段、最近时段起点、下次开盘与下次边界。A 股、港股和国内商品期货共用这一最终状态判断，数据提供器只解析供应商字段，不再推断开休市。节假日数据显式声明覆盖区间；未覆盖年份继续按工作日回退，并在脱敏诊断中暴露日历覆盖终点。
...
用户可主动执行“复制运行诊断信息”。装配层只把版本、配置、交易日与时段计数、日历覆盖终点、期货规则覆盖计数、分组/条目数量、行情状态计数和最近刷新结果交给领域格式化函数；报告不读取自选名称、证券代码、文件路径或工作区信息，也不会自动上传。原始错误详情继续保留在 FishStock 输出日志中，由用户自行决定是否查看或分享。
...
- 开市行情默认超过 120 秒标记过期。
- 休市由领域时段模型判定；提供器返回时间只用于验证行情是否足够新，不再决定开休市。
