...
腾讯股票或新浪期货行情若返回 HTTP 403，各自的提供器记录连续拒绝次数和下次自动探测时间；调度器在退避窗口内只跳过对应模块。退避从 15 分钟递增到最长 6 小时，手动刷新可以随时绕过等待；对应行情请求成功后立即清零。该状态只保存在当前扩展进程，不写入用户自选数据。
...
后台调度分别判断 Stock 自选涉及的市场和 Futures 自选涉及的市场，只有对应市场当天为交易日时才刷新该模块。扩展激活时的一次初始化刷新和用户手动刷新不受这一限制。当前交易日判断不包含盘中时段：股票午休/收盘以及期货按品种不同的夜盘时段仍由行情时间和过期状态反映，不直接阻断请求。
...
独立 VS Code Extension Host 冒烟测试覆盖扩展激活、Stock/Futures 视图聚焦、关键命令注册和全部展开命令执行。`pnpm run release:check` 把单元测试、Extension Host 冒烟、版本一致性、VSIX 文件清单与打包结果组合成统一发布门槛。
