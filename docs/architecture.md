...
│  └─ 分组 → 主连或月份合约
└─ Holdings（fishStock.holdings）
   └─ 币种 → A 股、港股或境内交易所 ETF 持仓
```

`Stock`、`Fund`、`Futures` 和 `Holdings` 是 FishStock 容器中的四个同级 View。股票和市场指数共享 Stock 分组；沪深交易所 ETF 使用 Fund；期货因交易时段、代码和行情字段不同使用 Futures；持仓按人民币和港币自动分组。前三个自选品类各自拥有命令、存储、视图状态和刷新状态，Holdings 使用独立存储，但复用 Stock/Fund 行情服务与缓存。

## 模块

| 模块 | 目录 | 责任 |
| --- | --- | --- |
| 领域模型 | `src/domain` | 股票、分组、持仓与行情结构；代码规范化；收益与持仓调仓计算；顺序调整；交易日历与休市判断；视图展示方式；行情页 URL 模板；脱敏诊断汇总与格式化 |
| 行情数据 | `src/data` | 提供器接口、腾讯股票/ETF 行情、新浪期货行情、北交所证券目录、节假日休市数据、缓存及请求合并 |
| 本地存储 | `src/storage` | 股票、基金、期货与持仓的独立版本化结构、视图展示方式、状态栏轮播分组与展示模式选择、校验和读写 |
| 刷新调度 | `src/services` | 自动刷新间隔、同一任务合并与交易时段边界监控 |
| 行情展示 | `src/ui/quoteTooltip.ts`、`src/ui/holdingTooltip.ts` | 自选行情与持仓收益的原生浮窗；共享价格、状态和时间格式 |
| Tree View | `src/ui/watchlistTreeProvider.ts`、`src/ui/holdingsTreeProvider.ts` | 自选分组树与按币种汇总的持仓树、状态文案和主题颜色 |
| 持仓管理器 | `src/ui/holdingsManagerPanel.ts` | 编辑器 Webview、批量草稿交互、搜索消息、行情预览与统一保存编排 |
| 状态栏 | `src/ui/statusBarController.ts` | 在一个状态栏项目中展示自选行情轮播或持仓盈亏轮播 |
| 命令 | `src/commands` | 装配股票/基金/期货自选命令，以及持仓增删改、清空、刷新、展示模式和行情页跳转 |
...
刷新读取本机自选与持仓代码，将规范化代码交给现有 `QuoteService`。持仓不会创建新的行情服务：股票持仓进入 Stock 的批次，境内 ETF 持仓进入 Fund 的批次，因此短缓存、在途请求合并、退避和交易时段判断完全复用。非强制刷新按最近一次成功请求的完成时间复用 10 秒内缓存；最近成功获取时间判断请求新鲜度，供应商行情时间只判断交易日归属和异常超前。提供器返回的原始字段必须经过解析和数值校验，UI 不直接理解第三方响应格式。

行情模型除现价和昨收/昨结外，还可携带今开、最高、最低、成交量、成交额、换手率、PE(TTM)、总市值、持仓量和交易所。股票浮窗根据规范化代码补充上交所、深交所、创业板、科创板、北交所或港交所；期货浮窗展示昨结、成交量、持仓量和交易所，不套用股票估值字段。Tree View 和状态栏调用同一个原生 `MarkdownString` 构建器；Webview 仅用于持仓批量管理，不替代日常行情展示。
...
持仓管理器通过消息向扩展进程并行调用 Stock 与 Fund 的现有搜索能力，只接收 A 股、港股和境内交易所 ETF。用户可在多轮搜索结果中多选标的，随后在同一表格内填写或修改多行数量与平均成本。页面只发送 `id`、规范化代码、数量和成本草稿；扩展侧只允许现有持仓或本次会话已确认的搜索结果，并重新校验正整数数量、正数成本、重复代码和不可变标的身份。全部行通过后调用仓库一次性替换；存储失败时不提前改变内存快照。新增标的按 Stock/Fund 分批强制补取一次，之后与自选代码共同进入原有自动刷新批次。

每一行还可在表格内展开调仓：页面只发送方向、变动数量和成交价，由扩展侧 `holdingAdjustment` 领域函数按移动加权平均成本算出新的数量与平均成本后回填草稿——加仓摊薄成本，减仓只改数量且成本不变，数量为 0 时从草稿移除该行。平均成本按 4 位小数记录。调仓不新增存储字段，也不绕过现有整批校验：预览失败会阻止应用，应用后仍需点击“保存全部”才写入；已实现盈亏只在预览和提示中返回，不落到存储或持仓汇总。Webview 不重复实现成本公式，预览与应用共用同一个领域函数。

管理器的 HTML、CSS 和脚本随扩展打包，使用 nonce CSP，不加载外部资源，也不能直接调用行情端点或 `globalState`。未保存草稿只保留在扩展进程内存；页面关闭时由原生模态提示保存、继续编辑或放弃。行情预览读取现有缓存，持仓市值为“现价 × 数量”，浮动盈亏为“(现价 - 平均成本) × 数量”，收益率为“浮动盈亏 ÷ 成本市值”。`closed` 和 `stale` 的有效价格仍参与计算并带状态标记，`error` 或无有效价格时返回不可计算。
...
持仓使用 `fishStock.holdings.v1`，状态栏展示模式使用 `fishStock.statusBarDisplay.v1`。持仓结构单独保存 `id`、`symbol`、`market`、`kind`、`name`、`quantity` 和 `averageCost`；数量必须是正安全整数，平均成本必须是有限正数，代码全局去重。仓库默认空白，无效状态安全回退为空列表。状态栏展示模式默认 `quotes`，可显式保存为 `holdings`。这些新键不迁移或改写既有自选键，也未启用 VS Code Settings Sync。

期货仓库为了复用版本 1 的最小结构，内部字段仍名为 `stocks`，但它只存放 `.CNF` 条目且位于独立命名空间。这是内部持久化兼容细节，不代表期货被放入 Stock View，也不是公开 JSON 导入导出格式。

`Stock`、`Fund`、`Futures` 与 `Holdings` 保持为四个可在 VS Code Views 菜单中独立显示或隐藏的原生 View。扩展清单以 `visibility: visible` 和 `initialSize` 权重 `2:1:1:1` 提供全新工作区的默认展开布局。View 隐藏期间的行情树更新会被合并，首次显示直接读取当前内存缓存。扩展不在启动阶段调用 `TreeView.reveal`，因此不会改变活动侧边栏，也不会在一个 View 展开时联动其他 View。VS Code 只在工作区首次见到 View 时采用清单默认值，之后的折叠状态和高度由原生布局保存，用户可拖动或双击分隔线重新分配。
...
用户可主动执行“复制运行诊断信息”。装配层只把版本、配置、交易日与时段计数、日历覆盖终点、期货规则覆盖计数、分组/条目数量、持仓币种分布、行情状态与过期原因计数和最近刷新结果交给领域格式化函数；报告不读取自选名称、证券代码、持仓数量、平均成本、市值、盈亏、文件路径或工作区信息，也不会自动上传。原始错误详情继续保留在 FishStock 输出日志中，由用户自行决定是否查看或分享。
...
- 状态栏默认每 5 秒在用户选中的分组内轮播行情，默认选择全部分组；用户可切换为逐条持仓盈亏轮播。两个模式都只读取内存行情，不触发网络请求，且始终共用一个状态栏项目。
...
- 持仓独立本地存储、数量与成本校验、默认空状态、收益计算和币种汇总
- 多行持仓草稿物化、现有标的身份保护、未知搜索结果拒绝、整批替换和存储失败回滚
...
- 股票、基金、期货与持仓命令 ID、四个 View 与扩展清单声明的一致性
- 状态栏行情/持仓展示模式的默认值、持久化和非法值回退
- 脱敏诊断的计数、格式与持仓隐私边界

独立 VS Code Extension Host 冒烟测试覆盖扩展激活、Stock/Fund/Futures/Holdings 打开、持仓管理器创建、关键命令注册和全部展开命令执行。`pnpm run release:check` 把单元测试、Extension Host 冒烟、版本一致性、VSIX 文件清单与打包结果组合成统一发布门槛。
