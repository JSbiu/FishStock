...
添加持仓时并行使用 Stock 与 Fund 的现有搜索能力，只接收 A 股、港股和境内交易所 ETF。用户选中标的后输入正整数数量和正数平均成本；保存后只为该标的触发一次强制补取，之后与自选代码共同进入原有自动刷新批次。持仓市值为“现价 × 数量”，浮动盈亏为“(现价 - 平均成本) × 数量”，收益率为“浮动盈亏 ÷ 成本市值”。`closed` 和 `stale` 的有效价格仍参与计算并带状态标记，`error` 或无有效价格时返回不可计算。
...
独立 VS Code Extension Host 冒烟测试覆盖扩展激活、Stock/Fund/Futures/Holdings 打开、关键命令注册和全部展开命令执行。`pnpm run release:check` 把单元测试、Extension Host 冒烟、版本一致性、VSIX 文件清单与打包结果组合成统一发布门槛。
