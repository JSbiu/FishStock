...
`Stock` 与 `Futures` 保持为两个可在 VS Code Views 菜单中独立显示或隐藏的原生 View。扩展清单将两者的首次可见性设为 `visible`，并以 `initialSize` 权重 `2:1` 分配初始高度；该声明只在视图首次出现且尚无已保存尺寸时生效，之后完全尊重 VS Code 保存的折叠、可见性和高度。两个视图都展开时，用户可双击分隔线调用 VS Code 原生均分行为。
