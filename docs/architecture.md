...
`Stock`、`Fund` 与 `Futures` 保持为三个可在 VS Code Views 菜单中独立显示或隐藏的原生 View。扩展清单以 `visibility: visible` 和 `initialSize` 权重 `2:1:1` 提供全新工作区的默认展开布局。View 隐藏期间的行情树更新会被合并，首次显示直接读取当前内存缓存。0.5.6 不在启动阶段调用 `TreeView.reveal`，因此不会改变活动侧边栏，也不会在一个 View 展开时联动其他 View。VS Code 只在工作区首次见到 View 时采用清单默认值，之后的折叠状态和高度由原生布局保存，用户可拖动或双击分隔线重新分配。
